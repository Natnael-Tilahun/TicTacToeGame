...
function Player(playerName, stepsArray, won) {
  ...
  this.checkWinner = function (res) {
    ...
    if (res.filter(checkRowWinner).length >= row) {
      return true;
    } else if (res.filter(checkColumnWinner).length >= row) return true;
    else if (res.filter(checkLeftDiagonalWinner).length >= row) return true;
    else if (res.filter(checkRightDiagonalWinner).length >= row) return true;
    return false;
  };
